"""
Step4: Background Extraction via Image Generation

This script takes the output of step2 (entries with identified moving objects) and
uses a Flux image generation model to produce a background-only image by removing
all subjects from the original frame.

While step3 extracts individual subject images, this step extracts the clean
background. Together they provide the decomposed components needed for downstream
video generation.

The generation is parallelized across multiple GPUs using multiprocessing.
...
Output (--output_dir):
    A directory where background images are saved, one per entry,
    named as: {videoid}.png
...
BACKGROUND_PROMPT = "Remove all subjects, keep only the background."
...
def collect_tasks(all_items: list, output_dir: str, base_seed: int) -> list:
    """
    Create one background-extraction task per input entry.
    ...
    for entry in all_items:
        ...
        output_path = os.path.join(output_dir, f"{videoid}.png")
...
def process_single_task(task_data: dict) -> str:
    """Run a single background extraction task and save the result."""
    global _process_model, _process_gpu_id
    ...
    try:
        ...
        # Generate the background-only image
...
def parse_args():
    p = argparse.ArgumentParser(description="Step4: Background extraction via Flux image generation")
    ...
    p.add_argument("--output_dir", type=str, required=True,
                   help="Directory to save extracted background images")
