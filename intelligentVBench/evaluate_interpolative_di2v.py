...
from all_prompts import interpolative_di2v
    ...
    while retry_count < max_retries:
        try:
            ...
            for i, (keyname, ref_img_path) in enumerate(ref_image_paths):
                ...
                if i == 0:
                    assert keyname == "first_frame"
                    user_content.append(
                        {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{cur_base64_image}"}}
                    )
                else:
                    assert keyname == "last_frame"
                    user_content.append({"type": "text", "text": "This is the second input image serving as the last frame:"})
                    user_content.append(
                        {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{cur_base64_image}"}}
                    )
...
def _process_single_row(row_idx, row, header, edited_video_path, max_tokens, file_parent_path=None):
    try:
        ref_img_paths = []
        for keyname in ["first_frame", "last_frame"]:
        ...
        system_prompt = interpolative_di2v
