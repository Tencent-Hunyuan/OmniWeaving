...
class HunyuanVideo_1_5_DiffusionTransformer(ModelMixin, ConfigMixin, PeftAdapterMixin):
    ...
    def __init__(
        ...
        self.deep_stack = deepstack
        if self.deep_stack is None:
            self.deep_stack = []
        self.deep_stack = list(self.deep_stack)
        if len(self.deep_stack) > 0:
            self.mm_in = TextProjection(
                text_states_dim,
                self.hidden_size,
                get_activation_layer("silu"),
                **factory_kwargs,
            )
            for n, p in self.mm_in.named_parameters():
    ...
    def forward(
        ...
        if all_stack_text_states is not None:
            all_stack_text_states_in = self.mm_in(all_stack_text_states)
        else:
        ...
        for index, block in enumerate(self.double_blocks):
            ...
            if all_stack_text_states_in is not None and index < len(all_stack_text_states_in):
                if self.freeze_main:
                    n_slice = all_stack_text_states_in[index].shape[-2]
                    n_keep = txt.shape[1] - n_slice

                    txt_front = txt[:, :n_keep]
                    txt_back = txt[:, -n_slice:]

                    updated_back = txt_back + all_stack_text_states_in[index].detach()

                    txt = torch.cat([txt_front, updated_back], dim=1)
                else:
                    local_this = txt[:, -all_stack_text_states_in[index].shape[-2]:].clone() + all_stack_text_states_in[index]
                    txt[:, -all_stack_text_states_in[index].shape[-2]:] = local_this
